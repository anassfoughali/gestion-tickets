...
import { FiX, FiTrendingUp, FiCheckCircle, FiAlertCircle, FiClock, FiLock } from "react-icons/fi";
...
const TechnicienModal = ({ tech, onClose }) => {
  ...
  useEffect(() => {
    if (!tech?.groupId) { setErrorMsg("ID technicien introuvable."); setLoading(false); return; }
    setLoading(true); setErrorMsg(null); setDetail(null);
  ...
  // ✅ Pie avec résolu + clôturé + ouvert séparés
  const pieData = detail ? [
    { name: "Résolus", value: detail.resolvedTickets || 0 },
    { name: "Clôturés",value: detail.cloturedTickets || 0 },
    { name: "Ouverts", value: detail.openTickets  || 0 },
    { name: "Autres", value: Math.max(0,
        detail.totalTickets
        - (detail.resolvedTickets || 0)
        - (detail.cloturedTickets || 0)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm"
      ...
      <div className="relative w-full max-w-lg mx-4 overflow-hidden bg-white shadow-2xl rounded-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white">
          ...
          <button onClick={onClose} className="flex items-center justify-center w-8 h-8 text-gray-400 transition rounded-full hover:bg-gray-100">
            <FiX size={18} />
          </button>
        </div>
        ...
        <div className="px-6 py-5">
          ...
          {!loading && !errorMsg && detail && (
            <>
              {/*  KPI cards : Résolus + Clôturés + Ouverts + Temps Moyen */}
              <div className="grid grid-cols-4 gap-2 mb-5">
                <div className="p-3 text-center border border-green-100 bg-green-50 rounded-xl">
                  <FiCheckCircle size={14} className="mx-auto mb-1 text-green-500" />
                  <p className="text-lg font-bold text-green-700">{detail.resolvedTickets || 0}</p>
                  <p className="text-xs text-green-500 mt-0.5">Résolus</p>
                </div>
                <div className="p-3 text-center border border-blue-100 bg-blue-50 rounded-xl">
                  <FiLock size={14} className="mx-auto mb-1 text-blue-500" />
                  <p className="text-lg font-bold text-blue-700">{detail.cloturedTickets || 0}</p>
                  <p className="text-xs text-blue-500 mt-0.5">Clôturés</p>
                </div>
                <div className="p-3 text-center border border-red-100 bg-red-50 rounded-xl">
                  <FiAlertCircle size={14} className="mx-auto mb-1 text-red-500" />
                  <p className="text-lg font-bold text-red-700">{detail.openTickets || 0}</p>
                  <p className="text-xs text-red-500 mt-0.5">Ouverts</p>
                </div>
                <div className="p-3 text-center border border-purple-100 bg-purple-50 rounded-xl">
                  <FiClock size={14} className="mx-auto mb-1 text-purple-500" />
                  <p className="text-lg font-bold text-purple-700">
                    {detail.avgResolutionTime > 0 ? `${detail.avgResolutionTime.toFixed(1)}h` : "—"}
                  </p>
                  <p className="text-xs text-purple-500 mt-0.5">Tps Moyen</p>
                </div>
              </div>
              ...
              <div>
                ...
                {pieData.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-40 text-sm text-gray-400">Aucun ticket</div>
                )}
              </div>
            </>
          )}
        ...
        <div className="flex justify-end px-6 py-3 border-t border-gray-100 bg-gray-50">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-white transition bg-indigo-600 rounded-lg hover:bg-indigo-700">
            Fermer
          </button>
        </div>
      </div>
...
const Techniciens = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Cliquez sur un technicien pour voir son détail</p>
          </div>
            ...
            <div className="p-6 bg-white border border-gray-100 shadow-sm rounded-xl">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">🖱️ Cliquez sur une barre pour voir le détail</p>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setActiveChart("temps")}
                    ...
                    }`}>⏱️ Temps Moyen</button>
                </div>
              </div>

              <ResponsiveContainer width="100%" height={Math.max(280, statsGlobales.length * 50)}>
                <BarChart data={statsGlobales} layout="vertical"
                  margin={{ top: 5, right: 30, left: 140, bottom: 5 }}
                  onClick={handleBarClick} style={{ cursor: "pointer" }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
                  <XAxis type="number" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                  <YAxis dataKey="name" type="category" tick={{ fontSize: 10, cursor: "pointer", fill: "#6366f1" }}
                    width={135} tickFormatter={(v) => v.length > 20 ? v.slice(0, 20) + "…" : v}
                    onClick={(data) => {
                      const found = techniciens.find(
                  ...
                  {activeChart === "tickets" ? (
                    <>
                      <Bar dataKey="resolus"  name="Résolus"  fill="#10b981" radius={[0, 4, 4, 0]} maxBarSize={18} />
                      <Bar dataKey="clotures" name="Clôturés" fill="#3b82f6" radius={[0, 4, 4, 0]} maxBarSize={18} />
                      <Bar dataKey="ouverts"  name="Ouverts"  fill="#ef4444" radius={[0, 4, 4, 0]} maxBarSize={18} />
                      <Bar dataKey="total"    name="Total"    fill="#6366f1" radius={[0, 4, 4, 0]} maxBarSize={18} />
                    </>
                  ) : (
                    <Bar dataKey="avgResolutionTime" name="Temps Moyen (h)" fill="#8b5cf6" radius={[0, 4, 4, 0]} maxBarSize={18} />
                  )}
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          {techniciens.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {techniciens.map((tech) => (
                <button key={tech.groupId} onClick={() => setPopupTech(tech)}
      ...
      {popupTech && <TechnicienModal tech={popupTech} onClose={() => setPopupTech(null)} />}
    </div>
  );
};
