...
const statusBadge = (status) => {
  ...
  if (s.includes("clot") || s.includes("ferm") || s.includes("sans solution"))
    return "bg-green-100 text-green-700";
  if (s.includes("cours") || s.includes("affect"))
...
const isResolu = (status) => {
  if (!status) return false;
  const s = status.toLowerCase();
  return s.includes("clot") || s.includes("ferm") || s.includes("sans solution");
};

const isOuvert = (status) => {
  if (!status) return false;
  const s = status.toLowerCase();
  return s.includes("ouvert") || s.includes("nouveau");
};

const isEnCours = (status) => {
  if (!status) return false;
  const s = status.toLowerCase();
  return s.includes("cours") || s.includes("affect");
};

const priorityBadge = (p) => {
  if (!p) return "bg-gray-50 text-gray-500";
  const pr = p.toLowerCase();
  if (pr === "critique") return "bg-red-50 text-red-600 border border-red-200";
  if (pr === "majeur")   return "bg-orange-50 text-orange-600 border border-orange-200";
  if (pr === "mineur")   return "bg-green-50 text-green-600 border border-green-200";
...
const Rapports = () => {
  ...
  // ✅ Liste unique des techniciens pour le select
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter((t) => {
      const matchStatus =
        ...
        (filterStatus === "ouvert"  && isOuvert(t.status))  ||
  ...
  //  Export Excel — exporte TOUS les tickets filtrés (pas seulement la page courante)
  const exportToExcel = () => {
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Génération et export des rapports — données SAP HANA
              </p>
            </div>
          ...
          {error && !loading && (
            <div className="flex items-center justify-between p-4 border border-red-200 bg-red-50 rounded-xl">
              ...
              <button onClick={fetchData} className="text-xs text-red-500 underline hover:text-red-700">
                Réessayer
              </button>
            </div>
          )}
                      ...
                      {currentRows.length === 0 ? (
                        ...
                        currentRows.map((t, idx) => (
                          <tr key={t.issueID || idx} className="transition hover:bg-gray-50">
                            <td className="py-3 font-mono text-xs font-semibold text-indigo-600">
                              {t.issueID}
                            </td>
                            <td className="py-3 text-gray-700">
                              <p className="truncate max-w-[160px]" title={t.briefDescription}>
                                {t.briefDescription}
                              </p>
                            </td>
                  ...
                  <div className="flex items-center justify-between pt-4 mt-4 border-t border-gray-100">
                    <p className="text-xs text-gray-400">
                      ...
                      <span className="font-semibold text-gray-600">
                        {Math.min(startIndex + ROWS_PER_PAGE, filteredTickets.length)}
                      </span>
                      {" sur "}
